...
CH9434_ISta CH9434D_CAN_GetITStatus (u32_t CAN_IT) {
    ...
    if ((CH9434ReadCANReg (CH9434D_CAN_INTENR) & CAN_IT) != RESET_T) {
        ...
        case CH9434D_CAN_IT_WKU:
            ISta = CheckITStatus (CH9434D_CAN_STATR, CAN_STATR_WKUI);
            break;

        case CH9434D_CAN_IT_SLK:
            ISta = CheckITStatus (CH9434D_CAN_STATR, CAN_STATR_SLAKI);
            break;

        case CH9434D_CAN_IT_EWG:
            ISta = CheckITStatus (CH9434D_CAN_ERRSR, CAN_ERRSR_EWGF);
            break;

        case CH9434D_CAN_IT_EPV:
            ISta = CheckITStatus (CH9434D_CAN_ERRSR, CAN_ERRSR_EPVF);
            break;

        case CH9434D_CAN_IT_BOF:
            ISta = CheckITStatus (CH9434D_CAN_ERRSR, CAN_ERRSR_BOFF);
            break;

        case CH9434D_CAN_IT_LEC:
            ISta = CheckITStatus (CH9434D_CAN_ERRSR, CAN_ERRSR_LEC);
            break;

        case CH9434D_CAN_IT_ERR:
            ISta = CheckITStatus (CH9434D_CAN_STATR, CAN_STATR_ERRI);
            break;
