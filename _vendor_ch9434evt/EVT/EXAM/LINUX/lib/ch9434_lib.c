/*
 * application library of SPI/I2C to Quad UARTs chip CH9434A/D/M, etc.
 *
...
int ch943x_ctrl_write(int fd, uint8_t cmd, uint32_t len, uint8_t *data)
{
...
int ch943x_ctrl_read(int fd, uint8_t cmd, uint32_t len, uint8_t *data)
{
	struct _ctrl_info *ctrlinfo;
	int ret;
...
int ch943x_single_gpioenable(int fd, uint8_t gpionumber, uint8_t enable)
{
	...
	if (chiptype == CHIP_CH9434D) {
		...
		if (ch943x_ctrl_read(
			    fd, CH9434_GPIO_FUNC_EN_0 + (gpionumber / 8),
			    1, &val2))
			return -ERROR_CODE1;
 ...
 * The function return 0 if success, others if fail.
 */
int ch943x_single_gpioconfig(int fd, uint8_t gpionumber, IODir dir,
			     IOMode mode)
{
	...
	if (chiptype == CHIP_CH9434D) {
		if (ch943x_ctrl_read(
			    fd, CH9434_GPIO_DIR_MOD_0 + (gpionumber / 2),
			    1, &val1))
			return -ERROR_CODE1;
		...
		if (ch943x_ctrl_read(
			    fd, CH9434_GPIO_DIR_MOD_0 + (gpionumber / 8),
			    1, &val3))
			return -ERROR_CODE1;
...
int ch943x_single_gpioset(int fd, uint8_t gpionumber, uint8_t value)
{
	...
	if (chiptype == CHIP_CH9434D) {
		...
		if (ch943x_ctrl_read(fd,
				     CH943X_GPIOVAL_REG + (gpionumber / 8),
				     1, &val))
			return -ERROR_CODE1;
		...
		if (ch943x_ctrl_write(
			    fd, CH943X_GPIOVAL_REG + (gpionumber / 8), 1,
			    &val))
			return -ERROR_CODE1;
	}
...
/**
 * ch943x_single_gpioget - Obtain the input level value of the GPIO
 * @fd: file descriptor of ch943x gpio device
 * @gpionumber: gpio number
 *
...
int ch943x_single_gpioget(int fd, uint8_t gpionumber)
{
	...
	if (chiptype == CHIP_CH9434D) {
		if (ch943x_ctrl_read(
			    fd, CH9434_GPIO_PIN_VAL_0 + (gpionumber / 8),
			    1, &val) < 0)
			return -ERROR_CODE1;
		retval = val > 0 ? 1 : 0;
	} else {
